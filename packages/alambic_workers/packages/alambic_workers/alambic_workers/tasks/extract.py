...
def _persist_extraction(doc_id, tx_id, account_id, indexes, summary) -> None:
    """Persiste le résumé sur le document et les index non-vides."""
    with session_scope() as s:
        doc = s.get(Document, doc_id)
        if doc is not None:
            doc.extraction_summary = summary
            # Après extraction, le document attend une validation humaine.
            # (L'auto-validation sur extraction_ok pourra être ajoutée plus tard.)
            doc.status = DocumentStatus.PENDING_VALIDATION.value
...
def extract_document(payload: dict) -> dict:
    ...
    with step(tx_id, PROCESS_EXTRACT, document_id=doc_id) as st:
        ...
        _persist_extraction(doc_id, tx_id, account_id, all_indexes, summary)
