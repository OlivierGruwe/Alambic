...
class FernetSecretProvider(SecretProvider):
    ...
    def is_encrypted(self, value: str) -> bool:
        """True si `value` est un token Fernet déchiffrable par une clé courante.

        Garde-fou anti-double-chiffrement : un token Fernet est déchiffrable et
        commence par 'gAAAAA' (version + timestamp encodés). On teste réellement
        le déchiffrement pour éviter les faux positifs.
        """
        if not value:
            return False
        try:
            self._fernet.decrypt(value.encode("ascii"))
            return True
