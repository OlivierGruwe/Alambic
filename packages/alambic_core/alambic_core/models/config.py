...
class Config(Base, TimestampMixin, AuditMixin):
    ...
    # (refus à l'ingestion) et apparaît grisée dans les listes.
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)

    # ── Complétude de dossier ────────────────────────────────────────────────
    # expected_doctypes : liste structurée des doctypes attendus avec leur
    # caractère obligatoire/optionnel. Format :
    #   [{"doctype_id": "...", "required": true}, ...]
    # Source de vérité pour la classification (restriction aux types attendus),
    # la complétude (implicite : active dès qu'un type est obligatoire), et le
    # blocage d'export.
    expected_doctypes: Mapped[list] = mapped_column(JSONB, nullable=False, default=list)
    # config_fields : champs propagés à la transaction (résolus une fois et posés
    # comme index metadata sur chaque document). Format :
    #   [{"field_name", "field_label", "source_type", "source_key", "default_value"}]
    config_fields: Mapped[list] = mapped_column(JSONB, nullable=False, default=list)
    # consolidation_ws : définitions de WS d'enrichissement (validation/complétion
    # d'un champ extrait via appel externe). Un champ de doctype les référence par
    # leur "name". Format : [{"name", "url", "method", "response_mapping", ...}]
    consolidation_ws: Mapped[list] = mapped_column(JSONB, nullable=False, default=list)
